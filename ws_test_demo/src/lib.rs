...
// Simple auth endpoint to get caller's principal for WebSocket
#[query]
fn get_websocket_principal() -> Principal {
    ic_cdk::caller()
}

// Method to add a test message (for demo purposes)
#[update]
fn add_test_message(text: String) -> Result<(), String> {
    format!("Message received: {}", text);
    Ok(())
}
